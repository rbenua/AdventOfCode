...
for line in f:
    ...
    rows.append(("?".join([map] * 5), [int(g) for g in groups.strip().split(",")] * 5))

def process(map, groups, prefix="", memo={}):
    tgroups = tuple(groups)
    if (map, tgroups) in memo:
        return memo[(map, tgroups)]
    if len(groups) == 0:
        if "#" not in map:
    ...
    if '.' in map[0:g] or (len(map) > g and map[g] == "#"):
        if(map[0] == "#"):
            ...
            memo[(map, tgroups)] = 0
            return 0
        res = process(map[1:], groups, prefix + ("." if len(map) > g else ""), memo)
        memo[(map, tgroups)] = res
        #print(f"process 3 {map}, {groups} = {res}, g={g}")
        return res
    if map[0] == '#':
        res = process(map[g+1:], groups[1:], prefix + "#" * g + ("." if len(map) > g else ""), memo)
        memo[(map, tgroups)] = res
        #print(f"process 4 {map}, {groups} = {res}")
        return res
    else:
        res = process(map[g+1:], groups[1:], prefix + "#" * g + ("." if len(map) > g else ""), memo) + process(map[1:], groups, prefix + ".", memo)
        memo[(map, tgroups)] = res
