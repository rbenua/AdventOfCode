...
def disas(op, arg):
    ...
    if func == bxc:
        ...
        if arg <= 3:
            ...
            sarg = ["A", "B", "C"][arg-4]
    if func == adv or func == bdv or func == cdv:
        sarg = "1 << " + sarg
    return f"{func.__name__} {sarg}"

for i in range(0, len(program), 2):
    print(f"{i:3}: {disas(program[i], program[i+1])}")


def trial(prev):
    global registers, pc, outputs
    for i in range(8):
        registers["A"] = prev + i
        registers["B"] = 0
        registers["C"] = 0
        ...
        if outputs == program:
            return prev + i
        if program[-len(outputs):] == outputs:
            res = trial((prev + i) * 8)
            if res is not None:
                return res
    return None
