...
def maxscore(t, sofar, mpos, mdone, epos, edone, visited):
    global total, cache, misses
    total += 1
    if (r := cache.get((t, sofar, mpos, mdone, epos, edone, visited))) is not None:
        return r
    ...
    if mdone == t:
        for (next, dist) in nzpaths[mpos]:
            ...
            if nmdone <= 26 and next not in visited:
                ...
                nbest = maxscore(min(nmdone, edone), score, next, nmdone, epos, edone, canonicalize(visited | {next}))
                if nbest > best:
                    best = nbest
    else:
        for (next, dist) in nzpaths[epos]:
            ...
            if nedone <= 26 and next not in visited:
                ...
                nbest = maxscore(min(mdone, nedone), score, mpos, mdone, next, nedone, canonicalize(visited | {next}))
                if nbest > best:
                    best = nbest

    cache[(t, sofar, mpos, mdone, epos, edone, visited)] = best
    return best

print(maxscore(0, 0, "AA", 0, "AA", 0, frozenset({"AA"})))
print(total, misses)
